...
export function CertificateDownloadButton({
  ...
  async function handleDownload() {
    ...
    try {
      const response = await fetch(`/api/certificate/${moduleId}/download`, {
        method: "GET",
      });
      ...
      link.download = response.headers
        ...
        ?.split("filename=")[1]
        ?.replaceAll("\"", "") ?? `certificate-module-${moduleId}.pdf`;
      ...
      setError("Unable to download certificate right now.");
    } finally {
